...
本文件覆盖 Unreal 项目识别和引擎版本判定的规则：如何找到 `.uproject`、如何判断是否为 C++ 项目、如何从 `EngineAssociation` 判定引擎版本。本文件是用户级唯一规范中 Unreal 引擎版本判定规则的详细承载位置。
...
Unreal 项目的根标志是 `.uproject` 文件。识别步骤：
...
- `EngineAssociation`：引擎版本标识，用于判定项目使用的 Unreal 引擎版本（详见下文）。
